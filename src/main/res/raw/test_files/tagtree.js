...
;(function($){
	...
	$.bootui.widget.tagTree.prototype = {
		...
		initialize: function(element, options) {
            ...
            var sng = $('<a class="tagtree-single" tabindex="-1"><span>全部</span><div class="clear-icon"><b></b></div><div class="down-icon"><b></b></div></a>');
            ctn.append(sng);
            var drop = $('<div class="tagtree-drop"></div>').hide();
            // 搜索框
            var searchhtml = $('<div class="tagtree-search"></div>');
            searchhtml.append($('<input type="text" class="tagtree-sipt" placeholder="查询节点">'));
            ...
            ctn.on('click', '.clear-icon', function(e){
                e.stopPropagation();
                $(element).val('');
                ctn.find('.tagtree-single span').html('全部');
                ctn.find('.tagtree-sipt').val('');
            ...
            function setOpt(data){
            	var defoption = $('<option value="">全部</option>');
