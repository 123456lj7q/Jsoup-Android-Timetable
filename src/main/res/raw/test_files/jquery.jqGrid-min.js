				...
				success 	: function(data) {
					...
					*/
					if ($.founded(data)) {
                        ...
                        //判断是否是动态列
                        var isExists = true;
                        if (options.agreeDynamic) {
                            //判断远程获取的数据和本地数据是否一致
                            //如果不一致，那么就要走本地数据，一致那就走远程数据
                            
                            //循环本地数据，将不需要隐藏的列添加到变量中，方便后面比较
                            var localData = [];
                            $.each(options.colModel, function (index, item) {
                                if (!item.hidden) {
                                    localData.push($.extend(true, {}, item));
                                }
                            });
                            //比较两个集合的长度，
                            if (localData.length > data.length) {
                                $.each(localData, function (index, colRow) {
                                	var flag = false;
                                    $.each(data || [], function (index, colModel) {
                        ...
                        if (!isExists) {
                            $.each(data || [], function (i, colRow) {
                                var isSet = false;
                                $.each(options.colModel, function (index, colModel) {
                                    if (colRow.index == colModel.index) {
                                        options.colModel[index].zdpz_id = colRow.zdpz_id;
                                        options.colModel[index].agreeDynamic = options.agreeDynamic;
                                    }
                                });
                            });
